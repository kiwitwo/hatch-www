 document.getElementById("footercontainer").innerHTML = `<div class="footer">
            <table>
                <tr class="foot">
                    <th>Us</th>
                    <th>Community</th>
                    <th>Resources</th>
                </tr>
                <tr class="foot">
                    <td><a href="/hatch-www/hatch-team">Hatch Team</a></td>
                    <td><a href="/hatch-www/guidelines">Hatch Guidelines</a></td>
                    <td><a href="/hatch-www/about">About</a></td>
                </tr>
                <tr class="foot">
                    <td><a href="/hatch-www/contact-us">Contact Us</a></td>
                    <td><a href="https://discord.gg/ENTDZdaGnD">Discord Server</a></td>
                    <td><a href="/hatch-www/privacy-policy">Privacy Policy</a></td>
                </tr>
                <tr class="foot">
                    <td><a href="https://github.com/hatchdotlol">Source Code</a></td>
                    <td><a href="https://social.hatch.lol">Social Media</a></td>
                    <td><a href="/hatch-www/terms-of-use">Terms of Use</a></td>
                </tr>
            </table>
        </div>
      <div id="disc">Hatch is not affiliated with Scratch, the Scratch Team, the Scratch Foundation, or TurboWarp.</div>`;
